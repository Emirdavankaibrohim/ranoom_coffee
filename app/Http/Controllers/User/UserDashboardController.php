...
class UserDashboardController extends Controller
{
    ...
    if ($request->oldImage != null) {
        $oldPath = public_path('customerProfile/' . $request->oldImage);
...
$taxRate    = $taxSetting ? $taxSetting->tax_rate : 0;
    ...
    public function paymentConfirm(Request $request)
    {
        ...
        foreach ($carts as $cart) {
            Order::create([
                ...
                'status'         => 1,
                'payment_method' => 'mobile', // QRIS payment
                'order_type'     => $request->orderType, // From user selection
                'size'           => $cart->size,
                'notes'          => $cart->notes, // Include notes from cart
